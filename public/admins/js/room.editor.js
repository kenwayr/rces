...
class RoomView {
    ...
    ImportSeatMatrix(matrix) {
        this.matrix = matrix.map((v) => v ? 1 : 0);
        this.DrawMatrix();
    }
    SetMatrix(matrix) {
        this.matrix = matrix;
        ...
        for(var i=0; i < this.matrix.length; i++) {
            for(var j=0; j < this.matrix[i].length; j++) {
                if(this.matrix[i][j] > 0)
                    this.DrawSeat( (this.config.cellPadding + this.config.cellWidth) * j + this.config.cellPadding, (this.config.cellPadding + this.config.cellHeight) * i + this.config.cellPadding, this.matrix[i][j]);
            }
        }
    }
}