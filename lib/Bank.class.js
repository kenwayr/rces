...
module.exports = class Bank {
    ...
    AddCandidate(candidateId) {
        this.clients.set(candidateId, { contributions: 0, score: 0, answers: [] });
    }
    ...
    async GenerateSets() {
        ...
        var questions = Array.from(this.bank.values());
    ...
    GetData() {
        for(var client of this.clients) {
            client.score = (client.contributions * 5) + client.answers.reduce( (a, v) => a + (v.status ? v.score : -(v.score/2)) );
        }
        return this.clients;
    }
}