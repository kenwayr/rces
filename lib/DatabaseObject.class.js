...
module.exports = class DatabaseObject {
    ...
    async LoadRooms() {
        var res = await this.db.collections.rooms.find();
        return res;
    }
    ...
    async SaveRooms(rooms) {
        var updates = rooms.map((room) => {
            return {
                "updateOne": {
                    "filter": { "_id": room.id || new ObjectID() },
    ...
    async LoadCourses() {
        var res = await this.db.collections.courses.find();
        return res;
    }
    ...
    async SaveCourses(courses) {
        var updates = courses.map((course) => {
            return {
                "updateOne": {
                    "filter": { "_id": course.id || new ObjectID() },
