...
module.exports = class DatabaseObject {
    constructor({hostname, port, database, accounts = 'accounts', students = 'students', courses = 'courses', rooms = 'rooms', sessions = 'sessions', resources = 'resources'}) {
        this.db = {
            config: {
                hostname: hostname,
                port: port,
    ...
    async Init() {
        ...
        this.db.collections.resources = this.db.instance.collection(this.db.config.resources);
    ...
    async SaveRooms(rooms) {
        var updates = rooms.map((room) => {
            return {
                "updateOne": {
                    "filter": { "_id": new ObjectID(room._id) || new ObjectID() },
    ...
    async LoadResources() {
        var res = await this.db.collections.resources.find({}).toArray();
        return res;
    }

    async SaveResources(resources) {
        var updates = resources.map((resource) => {
            return {
                "updateOne": {
                    "filter": { "_id": resource._id || new ObjectID() },
                    "update": { "$set": { "filename": resource.filename, "type": resource.type, "preferences": resource.preferences } },
                    "upsert": true
                }
            }
        })
        var res = null;
        if(updates.length > 0)
            res = await this.db.collections.resources.bulkWrite(updates);
        return res;
    }
